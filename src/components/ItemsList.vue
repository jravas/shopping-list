        ...
        <ul class="list-group">
          <li class="list-group-item d-flex justify-content-between align-items-center"
            ...
            <span class="badge">
              ...
              <span @click="addToStock(item)">
                <i class="fas fa-shopping-cart add-to-stock"></i>
              </span>
            </span>
          </li>
        </ul>
    ...
    <ul class="list" id="list-id">
      <transition-group name="list">
        <li v-for="(item, index) in data"
          ...
          <a class="list-item-element" @click="$emit('edit', {item: item, index: index})">
            {{ item.name }}
          </a>
    ...
    window.addEventListener('touchmove', (e) => {
      if (e.target.parentNode.classList.value === 'list-item') {
        ...
        if (touchStart < touchCurrent) {
          ...
          e.target.parentNode.getElementsByClassName('add-to-stock')[0].style.left = Math.abs(touchStart - touchCurrent) + 'px'
          e.target.parentNode.getElementsByClassName('add-to-stock')[0].style.opacity = Math.abs(touchStart - touchCurrent) / e.target.parentNode.getBoundingClientRect().width * 4
          // swiped to right
          if (Math.abs(touchStart - touchCurrent) > 150) {
            this.right = true
          } else {
            this.right = false
          }
          ...
          e.target.parentNode.getElementsByClassName('delete-item')[0].style.right = Math.abs(touchStart - touchCurrent) + 'px'
          e.target.parentNode.getElementsByClassName('delete-item')[0].style.opacity = Math.abs(touchStart - touchCurrent) / e.target.getBoundingClientRect().width * 4
          if (Math.abs(touchStart - touchCurrent) > 150) {
            this.left = true
          } else {
            this.left = false
